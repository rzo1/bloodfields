...
import com.example.armyclash.engine.UnitUpdater;
import com.example.armyclash.engine.World;
import com.example.armyclash.model.Army;
import com.example.armyclash.model.DamageModel;
import com.example.armyclash.model.HeroAura;
...
public final class UnitAI implements UnitUpdater {
    ...
    public void update(Unit u, GameState state, double dt) {
        ...
        if (state.world != null && !state.world.passableAt(u.x, u.y)) {
    ...
    static boolean lineCrossesImpassable(World w,
                                         com.example.armyclash.engine.StructureField structures,
    ...
    private static void spawnProjectile(Unit u, GameState state) {
        ...
        if (mag <= 1.0e-9) {
            ...
            dirX = dx / mag;
            dirY = dy / mag;
            vx = dirX * AiTuning.projectileSpeed;
            vy = dirY * AiTuning.projectileSpeed;
        }
        double startX = u.x;
        double startY = u.y;
        if (u.garrisoned && state.structures != null) {
            com.example.armyclash.engine.Structure host =
