...
import com.github.rzo1.bloodfields.ai.Pathfinding;
...
public final class Levels {
    ...
    public static List<Level> all() {
        return List.of(
                new Level(1, "Skirmish Line", 220, Levels::skirmishLine, Weather.CLEAR, "plains",
                        caps(UnitType.INFANTRY, 30)),
                new Level(2, "Pike Wall", 280, Levels::pikeWall, Weather.CLEAR, "plains",
                        caps(UnitType.INFANTRY, 20, UnitType.PIKEMAN, 15)),
                new Level(3, "Archers on the Flanks", 360, Levels::archersOnFlanks, Weather.CLEAR, "bridge",
                        caps(UnitType.ARCHER, 12)),
                new Level(4, "Cavalry Charge", 440, Levels::cavalryCharge, Weather.RAIN, "twin",
                        caps(UnitType.CAVALRY, 10)),
                new Level(5, "Mage Cabal", 520, Levels::mageCabal, Weather.FOG, "crossroads",
                        caps(UnitType.MAGE, 6, UnitType.ARCHER, 12)),
                new Level(6, "Stone Sentinels", 600, Levels::stoneSentinels, Weather.CLEAR, "woods",
                        caps(UnitType.GOLEM, 3, UnitType.ARCHER, 12)),
                new Level(7, "Healer's Stand", 700, Levels::healersStand, Weather.CLEAR, "mire",
                        caps(UnitType.HEALER, 6, UnitType.INFANTRY, 20)),
                new Level(8, "Shadow Cult", 800, Levels::shadowCult, Weather.NIGHT, "gauntlet",
                        caps(UnitType.NECROMANCER, 4, UnitType.ASSASSIN, 8)),
                new Level(9, "Siege Towers", 900, Levels::siegeTowers, Weather.CLEAR, "fortress_wall",
                        caps(UnitType.CATAPULT, 4, UnitType.ARCHER, 16, UnitType.GENERAL, 1)),
                new Level(10, "Final Fuckery", 2200, Levels::finalFuckery, Weather.NIGHT, "fortress_wall",
                        caps(UnitType.CATAPULT, 6, UnitType.MAGE, 8, UnitType.NECROMANCER, 4,
                                UnitType.ARCHER, 20, UnitType.GOLEM, 4, UnitType.GENERAL, 1)),
                new Level(11, "Simon's Nightmare", 3500, Levels::simonsNightmare, Weather.NIGHT, "fortress_wall",
                        java.util.Collections.emptyMap())
        );
    }

    private static Map<UnitType, Integer> caps(Object... pairs) {
        if (pairs.length % 2 != 0) {
            throw new IllegalArgumentException("caps() requires alternating type/limit pairs");
        }
        EnumMap<UnitType, Integer> m = new EnumMap<>(UnitType.class);
        for (int i = 0; i < pairs.length; i += 2) {
            m.put((UnitType) pairs[i], (Integer) pairs[i + 1]);
        }
        return m;
    }
    ...
    private static void simonsNightmare(Army enemy, GameState state, int width, int height) {
        double cx = width / 2.0;

        double pikeSpacing = 38.0;
        for (int i = 0; i < 32; i++) {
            double x = cx + (i - 15.5) * pikeSpacing;
            place(enemy, state, UnitType.PIKEMAN, x, 360.0);
        }
        double golemSpacing = 75.0;
        for (int i = 0; i < 16; i++) {
            double x = cx + (i - 7.5) * golemSpacing;
            place(enemy, state, UnitType.GOLEM, x, 305.0);
        }
        double archerSpacing = 38.0;
        for (int i = 0; i < 32; i++) {
            double x = cx + (i - 15.5) * archerSpacing;
            place(enemy, state, UnitType.ARCHER, x, 250.0);
        }
        double mageSpacing = 75.0;
        for (int i = 0; i < 16; i++) {
            double x = cx + (i - 7.5) * mageSpacing;
            place(enemy, state, UnitType.MAGE, x, 195.0);
        }
        double catapultSpacing = 100.0;
        for (int i = 0; i < 12; i++) {
            double x = cx + (i - 5.5) * catapultSpacing;
            place(enemy, state, UnitType.CATAPULT, x, 140.0);
        }
        double necroSpacing = 145.0;
        for (int i = 0; i < 8; i++) {
            double x = cx + (i - 3.5) * necroSpacing;
            place(enemy, state, UnitType.NECROMANCER, x, 90.0);
        }

        double dragonSpacing = 280.0;
        double dragonY = 50.0;
        for (int i = 0; i < 4; i++) {
            double x = cx + (i - 1.5) * dragonSpacing;
            place(enemy, state, UnitType.DRAGON, x, dragonY);
        }

        place(enemy, state, UnitType.GENERAL, cx - 200.0, 30.0);
        place(enemy, state, UnitType.GENERAL, cx + 200.0, 30.0);
        enemy.setHeroSkill(HeroSkill.BATTLE_LUST);
    }
}
