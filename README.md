...
java -jar target/bloodfields-1.1.2-all.jar
```

Java 21 required. The shade plugin produces `bloodfields-1.1.2-all.jar`
...
```bash
mvn -DskipTests package
java -cp target/bloodfields-1.1.2-all.jar com.github.rzo1.bloodfields.cli.CliMain levels
java -cp target/bloodfields-1.1.2-all.jar com.github.rzo1.bloodfields.cli.CliMain level 5
printf '%s\n' \
  ...
  '{"op":"state"}' \
  '{"op":"quit"}' \
  | java -cp target/bloodfields-1.1.2-all.jar com.github.rzo1.bloodfields.cli.CliMain play 1
```
