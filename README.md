...
java -jar target/bloodfields-1.0.0-all.jar
```

Java 21 required. The shade plugin produces `bloodfields-1.0.0-all.jar`
...
```bash
mvn -DskipTests package
java -cp target/bloodfields-1.0.0-all.jar com.github.rzo1.bloodfields.cli.CliMain levels
java -cp target/bloodfields-1.0.0-all.jar com.github.rzo1.bloodfields.cli.CliMain level 5
printf '%s\n' \
  ...
  '{"op":"state"}' \
  '{"op":"quit"}' \
  | java -cp target/bloodfields-1.0.0-all.jar com.github.rzo1.bloodfields.cli.CliMain play 1
```
...
- App version is set via the `jpackage.appVersion` Maven property
  ...
  which is why this is independent of the project's `1.0.0` version.
